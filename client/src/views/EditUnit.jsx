...
const EditUnit = (props) => {
  ...
  // SINGLE-STATE OBJECT for the update a miniature form
  ...
  const updateMiniature = (e) => {
    e.preventDefault()
    const updatedMini = {
      name: miniatureData.name,
      quantity: miniatureData.quantity,
      pointsValue: miniatureData.pointsValue,
      category: miniatureData.category,
      battleReady: miniatureData.battleReady,
      legends: miniatureData.legends
    }
    axios.put(`http://localhost:8001/api/miniatures/${id}/edit`, updatedMini)
      .then ((res) => {
        console.log("Unit Updated");
        setMiniatureData(updatedMini)
        navigate(`/miniatures/${miniatureData._id}`)
      })
      .catch ((err) => {
  ...
  return (
    <div className="main-body">
      <div className="main-body__top">
        ...
        <h2 className="faction-banner__heading">Edit Unit</h2>
      </div>
      <div className="add-unit__body">
        <Link to={`/miniatures/${id}`}><p className='body-text'>Back to Faction</p></Link>
        <div className="unit-form__wrapper">
          <form onSubmit={updateMiniature} className='unit-form'>
            ...
            <div className="input-wrapper">
              ...
              <select name="category" id="category" value={miniatureData.category} onChange={categoryHandler}>
                ...
                <option value="DedicatedTransport">DedicatedTransport</option>
